...
async def test_get_sensor(  # pylint: disable=too-many-statements
    ...
    async with aiohttp.ClientSession() as session:
        ...
        assert response.timestamp_utc == datetime(2022, 11, 5, 16, 37, 3, tzinfo=timezone.utc)
        assert response.data_timestamp_utc == datetime(2022, 11, 5, 16, 36, 21, tzinfo=timezone.utc)
        ...
        assert response.sensor.date_created_utc == datetime(2021, 9, 29, 22, 46, 14, tzinfo=timezone.utc)
        ...
        assert response.sensor.last_modified_utc == datetime(2021, 10, 30, 22, 27, 9, tzinfo=timezone.utc)
        assert response.sensor.last_seen_utc == datetime(2022, 11, 5, 16, 36, 2, tzinfo=timezone.utc)
        ...
        assert response.sensor.stats.timestamp_utc == datetime(2022, 11, 5, 16, 36, 2, tzinfo=timezone.utc)
        ...
        assert response.sensor.stats_a.timestamp_utc == datetime(2022, 11, 5, 16, 36, 2, tzinfo=timezone.utc)
        ...
        assert response.sensor.stats_b.timestamp_utc == datetime(2022, 11, 5, 16, 36, 2, tzinfo=timezone.utc)
...
async def test_get_sensors(aresponses: ResponsesMockServer) -> None:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        assert response.api_version == "V1.0.11-0.0.41"
        assert response.timestamp_utc == datetime(2022, 11, 3, 19, 26, 29, tzinfo=timezone.utc)
        assert response.data_timestamp_utc == datetime(2022, 11, 3, 19, 25, 31, tzinfo=timezone.utc)
